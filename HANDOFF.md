...
- 当前接收现存的本地文件和文件夹；同一路径去重，最多 20 项。
- 搁板内项目拖回搁板会被识别为原项目并拒绝，不会生成重复引用。
- 普通文件退出后清空；固定文件跨退出和重启恢复，并且不参与容量自动替换。
- 容量不足时替换最早的未固定文件，替换后可撤销；未固定文件不足时整批拒绝。
- 文件拖到菜单栏图标时，即使面板关闭也会自动展开接收界面。
- 状态栏图标保持 18 pt，拖拽、悬停和点击命中宽度扩大到 32 pt。
- 拖入完成后面板自动收起；有内容时悬停图标可展开并向外拖出。
- 重复、无效或容量不足等提示提供“返回”，也可再次点击状态栏图标直接关闭。
