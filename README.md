...
- 没有普通主窗口和 Dock 图标，只有菜单栏入口与临时浮动搁板。
- 点击图标管理文件；有内容时悬停图标可展开；面板关闭时把文件拖到图标会自动展开。
- 拖入后只保存原文件引用，不复制、移动或上传文件内容。
- 不记录剪贴板历史，不做搜索、分类、同步或资料库。

## 当前状态

macOS 原生核心交互闭环已经可用，并在本轮由用户确认“整体效果和功能没问题”。当前实现包括菜单栏拖入唤出、放入后自动收起、悬停取出、水平文件列表、滚轮映射、系统文件图标、原生玻璃材质、菜单栏选中反馈和自适应面板锚点。
